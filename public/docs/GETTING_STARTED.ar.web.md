...
- لا يُنشأ أي شيء ولا يُعدَّل في نظام التتبع لديك حتى ترُدّ بـ <bdi dir="ltr">`FILE IT`</bdi> تمامًا. و«نعم» أو «تفضل» ليست تأكيدًا. وقد يوجّه BugIt إليه قبل ذلك استعلامات للقراءة فقط، باستخدام رمزك المحفوظ، للبحث عن التكرارات ولمعرفة الحقول التي يقبلها المشروع.
