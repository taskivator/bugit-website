...
/* Right-to-left scripts. `dir` is set from this on every language change, because an RTL
   ...
   the wrong side. */
/* WHICH LANGUAGES WE SHIP, ASKED OF THE CATALOGUE RATHER THAN THE DICTIONARY (CR-08-F01).
   The prototype-safe membership check was right and was asked in the wrong place: `i18n` is
   built as this file is read, ten locales near the top and ARABIC about two thousand lines
   below, in the generated override block. The initializer that resolves a returning visitor's
   stored choice runs between the two, so `hasLang('ar')` was false at the only moment it was
   ever asked. Every Arabic reader who had already chosen Arabic was handed English, on every
   load, and the language control reported English as the current language.
   `languages` is a literal declared above: complete before a single statement runs, and a Set
   built from it answers `has('__proto__')` with false, which is what CR-08-F01 was about.
   Derived, never listed, so a twelfth language cannot be half-added. */
const SHIPPED_LANGS=new Set(languages.map(function(l){return l[0]}));
...
let currentLang=(function(){
  ...
  if(chosen&&how==='user'&&SHIPPED_LANGS.has(chosen))return chosen;
  try{
    ...
    for(var i=0;i<tags.length;i++){
      ...
      if(!tag)continue;
      if(SHIPPED_LANGS.has(tag))return tag;
      var base=tag.split('-')[0];
      if(SHIPPED_LANGS.has(base))return base;
...
const reqFaq={
 en:["What do I need to run BugIt?","BugIt runs inside Visual Studio Code with GitHub Copilot Chat, the Claude extension, or another assistant and a recent version of Python (3.10 to 3.14). Windows 11 is fully supported and release-validated; macOS and Linux are supported but currently in Preview while validation broadens. BugIt does not include an AI model or subscription; you bring your own."],
 de:["Was brauche ich, um BugIt zu nutzen?","BugIt läuft in Visual Studio Code mit GitHub Copilot Chat, der Claude-Erweiterung oder einem anderen Assistenten und einer aktuellen Python-Version (3.10-3.14). Windows 11 wird vollständig unterstützt und ist release-validiert; macOS und Linux werden unterstützt, befinden sich aber derzeit in der Preview, während die Validierung ausgeweitet wird. BugIt enthält kein KI-Modell und kein Abo. Das bringen Sie selbst mit."],
 es:["¿Qué necesito para usar BugIt?","BugIt funciona dentro de Visual Studio Code con GitHub Copilot Chat, la extensión de Claude u otro asistente y una versión reciente de Python (3.10-3.14). Windows 11 es totalmente compatible y está validado para producción; macOS y Linux son compatibles, pero actualmente están en Preview mientras se amplía la validación. BugIt no incluye ningún modelo de IA ni suscripción: usas la tuya."],
 fr:["De quoi ai-je besoin pour utiliser BugIt ?","BugIt fonctionne dans Visual Studio Code avec GitHub Copilot Chat, l’extension Claude ou un autre assistant et une version récente de Python (3.10-3.14). Windows 11 est entièrement pris en charge et validé pour la production ; macOS et Linux sont pris en charge mais actuellement en Preview, le temps d’élargir la validation. BugIt n’inclut ni modèle d’IA ni abonnement : vous utilisez le vôtre."],
 it:["Cosa serve per usare BugIt?","BugIt funziona in Visual Studio Code con GitHub Copilot Chat, l’estensione Claude o un altro assistente e una versione recente di Python (3.10-3.14). Windows 11 è pienamente supportato e validato per il rilascio; macOS e Linux sono supportati ma attualmente in Preview mentre la validazione si amplia. BugIt non include alcun modello di IA né abbonamento: usi il tuo."],
 "pt-br":["O que preciso para usar o BugIt?","O BugIt funciona no Visual Studio Code com o GitHub Copilot Chat, a extensão do Claude ou outro assistente e uma versão recente do Python (3.10-3.14). O Windows 11 é totalmente compatível e validado para produção; macOS e Linux são compatíveis, mas atualmente estão em Preview enquanto a validação é ampliada. O BugIt não inclui modelo de IA nem assinatura. Você usa a sua."],
 ja:["BugItの利用に何が必要ですか？","BugItは Visual Studio Code 上で、GitHub Copilot Chat、Claude 拡張機能、または他のアシスタントと、最近のバージョンの Python（3.10-3.14）とともに動作します。Windows 11 は完全対応かつリリース検証済みです。macOS と Linux も対応していますが、検証範囲の拡大中のため現在はプレビュー扱いです。BugIt には AI モデルやサブスクリプションは含まれません。ご自身のものをご利用ください。"],
 ko:["BugIt을 사용하려면 무엇이 필요한가요?","BugIt은 Visual Studio Code에서 GitHub Copilot Chat, Claude 확장, 또는 다른 어시스턴트와 최신 버전의 Python(3.10-3.14)과 함께 실행됩니다. Windows 11은 완전히 지원되며 출시 검증을 마쳤습니다. macOS와 Linux도 지원되지만, 검증 범위를 넓히는 동안 현재는 프리뷰 상태입니다. BugIt에는 AI 모델이나 구독이 포함되어 있지 않으며, 직접 준비한 것을 사용합니다."],
 zh:["运行 BugIt 需要什么？","BugIt 在 Visual Studio Code 中运行，需要 GitHub Copilot Chat、Claude 扩展或其他助手，以及较新版本的 Python（3.10-3.14）。Windows 11 已获完全支持并通过发布验证；macOS 和 Linux 也受支持，但在验证范围扩大期间目前为预览（Preview）状态。BugIt 不含 AI 模型或订阅，请使用你自己的。"],
 ru:["Что нужно для работы BugIt?","BugIt работает в Visual Studio Code с GitHub Copilot Chat, расширением Claude или другим ассистентом и актуальной версией Python (3.10-3.14). Windows 11 полностью поддерживается и проверена для релиза; macOS и Linux поддерживаются, но сейчас находятся в статусе Preview, пока расширяется проверка. BugIt не включает модель ИИ или подписку. Вы используете свою."],
  ar: [
  "ما الذي أحتاجه لتشغيل BugIt؟",
  "يعمل BugIt داخل Visual Studio Code مع GitHub Copilot Chat أو امتداد Claude أو مساعد آخر، إلى جانب إصدار حديث من Python (3.10 إلى 3.14). نظام Windows 11 مدعوم بالكامل ومُعتمَد للإصدار؛ نظاما macOS وLinux مدعومان لكنهما حاليًا في وضع Preview أثناء توسيع التحقق. لا يتضمن BugIt نموذج ذكاء اصطناعي أو اشتراكًا؛ أنت تستخدم نموذجك الخاص."
]
};
...
const docRoutes=['docs','docs/getting-started','docs/user-guide','docs/overview','docs/license','docs/privacy','docs/security','docs/refund','docs/commerce','docs/faq','support'];
...
const docDownloadLabels={
  ...
  ko:{userGuide:"사용자 가이드",overview:"QA 에이전트 개요",ugDesc:"설치부터 사용까지 전체 단계별 가이드.",ovDesc:"BugIt QA 에이전트에 대한 간결한 개요.",preview:"미리보기",download:"다운로드"},
...
const _siteMeta={
  ...
  ko:{title:'BugIt | VS Code용 QA 버그 제출 에이전트',description:'BugIt은 대략적인 테스트 메모를 검토된 깔끔한 버그 티켓으로 변환하여 승인 후 트래커에 등록합니다.'},
...
function renderDocRoute(){
  ...
  const titles={docs:d.homeTitle,'docs/getting-started':dl.userGuide,'docs/user-guide':dl.userGuide,'docs/overview':dl.overview,'docs/license':d.licenseTitle,'docs/privacy':d.privacyTitle,'docs/security':d.securityTitle,'docs/refund':d.refundTitle,'docs/commerce':d.commerceTitle,'docs/faq':d.faqTitle,support:d.supportTitle};
  let body='',lede='';
  if(r==='docs'){
    const dl=docDownloadLabels[lang]||docDownloadLabels.en;
    ...
    const shelf=[['#/docs/license',labels.license,labels.licenseDesc],
                 ...
                 ['#/docs/security',labels.security,labels.securityDesc],
  ...
  }else if(r==='docs/security'){
    body=`<div id="securityText" class="license-doc" aria-busy="true">${docSkeleton()}</div>`;
  ...
  if(r==='docs/getting-started'||r==='docs/user-guide'||r==='docs/overview'){
    ...
    // Until 2026-09-24 SECURITY.md was reachable only as a raw file linked from the Overview:
    // unstyled, English in every language, with no navigation back. It is a document page now,
    // loaded exactly like the privacy statement, with English as the fallback.
    const box=document.getElementById('securityText');
    const urls=lang==='en'?['/public/docs/SECURITY.md']:['/public/docs/SECURITY.'+lang+'.md','/public/docs/SECURITY.md'];
    fetchFirstText(urls)
      .then(txt=>{if(box){box.innerHTML=formatMarkdownDoc(txt);box.removeAttribute('aria-busy');docReadingReady(token);}})
      .catch(()=>{docErrorInto(box,lang,'security')});
  }else if(r==='docs/refund'){
...
/* TRANSLATE WHAT WAS JUST INSERTED, NOT THE WHOLE PAGE. renderAccount called applyLang() to put
   words into the account menu it had built, and applyLang() also re-renders the open document:
   a signed-in reader of /privacy saw the page blank, reload and jump to the top when the
   session check answered, and every document was fetched twice. These are the same three
   substitutions applyLang makes, limited to the nodes renderAccount created. */
function localizeIn(root){if(!root)return;var dict=i18n[currentLang]||i18n.en;var els=[].slice.call(root.querySelectorAll('[data-t],[data-html],[data-t-aria]'));if(root.matches&&root.matches('[data-t],[data-html],[data-t-aria]'))els.push(root);els.forEach(function(el){var v;if(el.dataset.t){v=get(dict,el.dataset.t);if(v!==undefined)el.textContent=v}if(el.dataset.html){v=get(dict,el.dataset.html);if(v!==undefined)el.innerHTML=v}if(el.dataset.tAria){v=get(dict,el.dataset.tAria);if(v!==undefined)el.setAttribute('aria-label',v)}})}
function renderAccount(name){var slot=document.getElementById('authSlot'),list=document.getElementById('acctList'),label=document.getElementById('acctLabel');if(list)list.innerHTML=acctListHtml();if(label){if(name){label.textContent=name;label.removeAttribute('data-t');}else{label.setAttribute('data-t','account.myAccount');}}var mm=document.getElementById('mmAcct');if(mm){mm.innerHTML=acctListHtml();if(name){var nm=document.createElement('div');nm.className='mm-acct-name';nm.textContent=name;mm.insertBefore(nm,mm.firstChild);}mm.hidden=false;}var ms=document.querySelector('.mm-signin');if(ms)ms.hidden=true;if(slot)slot.dataset.state='in';localizeIn(list);localizeIn(label);localizeIn(mm);initAcctMenu();}
...
const consentI18n = {
  ...
  en:{title:'Cookies and measurement',body:'We use essential cookies to run this site, plus a cookieless Cloudflare measurement of page performance that does not track you across sites. With your permission we also use Google cookies to measure our advertising. You can change your choice at any time.',privacyLink:'Read our Privacy Policy',manage:'Manage preferences',save:'Save preferences',reject:'Reject non-essential',accept:'Accept all',essential:'Essential',essentialDesc:'Required for the site to function.',always:'Always on',advertising:'Advertising',advertisingDesc:'Helps us understand whether our advertising leads to purchases.',advMeasureTitle:'Advertising measurement',advProviderLabel:'Provider',advProvider:'Google',advPurposeLabel:'Purpose',advPurpose:'Advertising performance and purchase attribution',advInfoLabel:'Information that may be processed',advInfo:'Advertising interaction information, purchase value, currency, and a non personal order reference',prefsLink:'Cookie preferences'},
  de:{title:'Cookies und Messung',body:'Wir verwenden notwendige Cookies für den Betrieb der Website sowie eine cookielose Cloudflare-Messung der Seitenleistung, die Sie nicht websiteübergreifend verfolgt. Mit Ihrer Einwilligung nutzen wir außerdem Google-Cookies, um unsere Werbung zu messen. Sie können Ihre Auswahl jederzeit ändern.',privacyLink:'Datenschutzerklärung lesen',manage:'Einstellungen verwalten',save:'Einstellungen speichern',reject:'Nicht notwendige ablehnen',accept:'Alle akzeptieren',essential:'Notwendig',essentialDesc:'Für den Betrieb der Website erforderlich.',always:'Immer aktiv',advertising:'Werbung',advertisingDesc:'Hilft uns zu verstehen, ob unsere Werbung zu Käufen führt.',advMeasureTitle:'Werbemessung',advProviderLabel:'Anbieter',advProvider:'Google',advPurposeLabel:'Zweck',advPurpose:'Werbeleistung und Kaufzuordnung',advInfoLabel:'Möglicherweise verarbeitete Informationen',advInfo:'Informationen zu Werbeinteraktionen, Kaufwert, Währung und eine nicht personenbezogene Bestellreferenz',prefsLink:'Cookie-Einstellungen'},
  es:{title:'Cookies y medición',body:'Usamos cookies esenciales para el funcionamiento del sitio y una medición de rendimiento de Cloudflare sin cookies que no te rastrea de un sitio a otro. Con tu permiso, también usamos cookies de Google para medir nuestra publicidad. Puedes cambiar tu elección en cualquier momento.',privacyLink:'Leer nuestra Política de Privacidad',manage:'Gestionar preferencias',save:'Guardar preferencias',reject:'Rechazar no esenciales',accept:'Aceptar todo',essential:'Esenciales',essentialDesc:'Necesarias para el funcionamiento del sitio.',always:'Siempre activas',advertising:'Publicidad',advertisingDesc:'Nos ayuda a saber si nuestra publicidad genera compras.',advMeasureTitle:'Medición de publicidad',advProviderLabel:'Proveedor',advProvider:'Google',advPurposeLabel:'Finalidad',advPurpose:'Rendimiento publicitario y atribución de compras',advInfoLabel:'Información que puede procesarse',advInfo:'Información sobre la interacción con anuncios, el valor de la compra, la moneda y una referencia de pedido no personal',prefsLink:'Preferencias de cookies'},
  fr:{title:'Cookies et mesure',body:'Nous utilisons des cookies essentiels pour faire fonctionner le site, ainsi qu’une mesure de performance Cloudflare sans cookie qui ne vous suit pas d’un site à l’autre. Avec votre accord, nous utilisons aussi des cookies Google pour mesurer notre publicité. Vous pouvez modifier votre choix à tout moment.',privacyLink:'Lire notre politique de confidentialité',manage:'Gérer les préférences',save:'Enregistrer les préférences',reject:'Refuser les non essentiels',accept:'Tout accepter',essential:'Essentiels',essentialDesc:'Nécessaires au fonctionnement du site.',always:'Toujours actifs',advertising:'Publicité',advertisingDesc:'Nous aide à comprendre si notre publicité génère des achats.',advMeasureTitle:'Mesure publicitaire',advProviderLabel:'Fournisseur',advProvider:'Google',advPurposeLabel:'Finalité',advPurpose:'Performance publicitaire et attribution des achats',advInfoLabel:'Informations susceptibles d’être traitées',advInfo:'Informations sur l’interaction publicitaire, montant de l’achat, devise et une référence de commande non personnelle',prefsLink:'Préférences des cookies'},
  it:{title:'Cookie e misurazione',body:'Usiamo cookie essenziali per far funzionare il sito e una misurazione delle prestazioni di Cloudflare senza cookie che non ti traccia da un sito all’altro. Con il tuo consenso usiamo anche cookie di Google per misurare la nostra pubblicità. Puoi cambiare la tua scelta in qualsiasi momento.',privacyLink:'Leggi la nostra Informativa sulla privacy',manage:'Gestisci preferenze',save:'Salva preferenze',reject:'Rifiuta non essenziali',accept:'Accetta tutto',essential:'Essenziali',essentialDesc:'Necessari al funzionamento del sito.',always:'Sempre attivi',advertising:'Pubblicità',advertisingDesc:'Ci aiuta a capire se la nostra pubblicità genera acquisti.',advMeasureTitle:'Misurazione pubblicitaria',advProviderLabel:'Fornitore',advProvider:'Google',advPurposeLabel:'Finalità',advPurpose:'Prestazioni pubblicitarie e attribuzione degli acquisti',advInfoLabel:'Informazioni che possono essere trattate',advInfo:'Informazioni sull’interazione con gli annunci, valore dell’acquisto, valuta e un riferimento d’ordine non personale',prefsLink:'Preferenze cookie'},
  'pt-br':{title:'Cookies e medição',body:'Usamos cookies essenciais para o funcionamento do site e uma medição de desempenho da Cloudflare sem cookies que não rastreia você entre sites. Com sua permissão, também usamos cookies do Google para medir nossa publicidade. Você pode mudar sua escolha a qualquer momento.',privacyLink:'Ler nossa Política de Privacidade',manage:'Gerenciar preferências',save:'Salvar preferências',reject:'Recusar não essenciais',accept:'Aceitar tudo',essential:'Essenciais',essentialDesc:'Necessários para o funcionamento do site.',always:'Sempre ativos',advertising:'Publicidade',advertisingDesc:'Ajuda-nos a entender se a nossa publicidade gera compras.',advMeasureTitle:'Medição de publicidade',advProviderLabel:'Provedor',advProvider:'Google',advPurposeLabel:'Finalidade',advPurpose:'Desempenho da publicidade e atribuição de compras',advInfoLabel:'Informações que podem ser processadas',advInfo:'Informações sobre a interação com anúncios, valor da compra, moeda e uma referência de pedido não pessoal',prefsLink:'Preferências de cookies'},
  ja:{title:'Cookieと計測',body:'当サイトの動作に必要なCookieと、Cookieを使わず、サイトをまたいで追跡しないCloudflareのパフォーマンス計測を使用します。ご同意いただいた場合は、広告の効果測定のためにGoogleのCookieも使用します。設定はいつでも変更できます。',privacyLink:'プライバシーポリシーを読む',manage:'設定を管理',save:'設定を保存',reject:'必須以外を拒否',accept:'すべて許可',essential:'必須',essentialDesc:'サイトの動作に必要です。',always:'常に有効',advertising:'広告',advertisingDesc:'広告が購入につながっているかどうかの把握に役立ちます。',advMeasureTitle:'広告計測',advProviderLabel:'提供者',advProvider:'Google',advPurposeLabel:'目的',advPurpose:'広告のパフォーマンスと購入のアトリビューション',advInfoLabel:'処理される可能性のある情報',advInfo:'広告操作に関する情報、購入金額、通貨、および個人を特定しない注文参照',prefsLink:'Cookie設定'},
  ko:{title:'쿠키 및 측정',body:'사이트 운영에 필요한 필수 쿠키와, 쿠키를 사용하지 않고 사이트 간에 추적하지 않는 Cloudflare 성능 측정을 사용합니다. 동의하시면 광고 성과 측정을 위해 Google 쿠키도 사용합니다. 선택은 언제든지 변경할 수 있습니다.',privacyLink:'개인정보처리방침 보기',manage:'기본 설정 관리',save:'기본 설정 저장',reject:'필수 외 거부',accept:'모두 허용',essential:'필수',essentialDesc:'사이트 작동에 필요합니다.',always:'항상 켜짐',advertising:'광고',advertisingDesc:'광고가 구매로 이어지는지 파악하는 데 도움이 됩니다.',advMeasureTitle:'광고 측정',advProviderLabel:'제공자',advProvider:'Google',advPurposeLabel:'목적',advPurpose:'광고 성과 및 구매 기여도 분석',advInfoLabel:'처리될 수 있는 정보',advInfo:'광고 상호작용 정보, 구매 금액, 통화, 개인정보가 아닌 주문 참조',prefsLink:'쿠키 기본 설정'},
  zh:{title:'Cookie 与衡量',body:'我们使用必要的 Cookie 来运行本网站，以及不使用 Cookie、不跨网站追踪你的 Cloudflare 性能衡量。在你同意后，我们还会使用 Google 的 Cookie 来衡量广告效果。你可以随时更改你的选择。',privacyLink:'阅读我们的隐私政策',manage:'管理偏好设置',save:'保存偏好设置',reject:'拒绝非必要',accept:'全部接受',essential:'必要',essentialDesc:'网站运行所必需。',always:'始终开启',advertising:'广告',advertisingDesc:'帮助我们了解广告是否带来购买。',advMeasureTitle:'广告衡量',advProviderLabel:'提供方',advProvider:'Google',advPurposeLabel:'用途',advPurpose:'广告效果与购买归因',advInfoLabel:'可能处理的信息',advInfo:'广告互动信息、购买金额、币种以及不含个人信息的订单编号',prefsLink:'Cookie 偏好设置'},
  ru:{title:'Файлы cookie и измерение',body:'Мы используем необходимые файлы cookie для работы сайта, а также измерение производительности страниц Cloudflare без файлов cookie, которое не отслеживает вас на других сайтах. С вашего согласия мы также используем файлы cookie Google для оценки нашей рекламы. Вы можете изменить свой выбор в любое время.',privacyLink:'Читать нашу Политику конфиденциальности',manage:'Управление настройками',save:'Сохранить настройки',reject:'Отклонить необязательные',accept:'Принять все',essential:'Необходимые',essentialDesc:'Необходимы для работы сайта.',always:'Всегда включены',advertising:'Реклама',advertisingDesc:'Помогает нам понять, приводит ли наша реклама к покупкам.',advMeasureTitle:'Измерение рекламы',advProviderLabel:'Поставщик',advProvider:'Google',advPurposeLabel:'Цель',advPurpose:'Эффективность рекламы и атрибуция покупок',advInfoLabel:'Информация, которая может обрабатываться',advInfo:'Информация о взаимодействии с рекламой, сумма покупки, валюта и обезличенный номер заказа',prefsLink:'Настройки cookie'}
};
...
/* Consent banner controller. The banner (index.html) is denied-by-default and only
   ...
   Essential-only never grants advertising.

   THERE IS NO ANALYTICS SWITCH, AND analytics_storage IS ALWAYS WRITTEN false. The panel used
   to offer one, and it controlled nothing: the only Google tag on this site is the Ads tag,
   which loads only when ad_storage is granted, and there is no Google Analytics. The one
   measurement that does run, the Cloudflare Web Analytics beacon, is cookieless, injected at
   the edge, allowed before any choice on purpose (scripts/check-consent-network.mjs) and
   disclosed in the privacy policy as always on, so no switch here could turn it off. A toggle
   that changes nothing tells the visitor they have a choice they do not have. The FIELD stays
   in the cookie because it is part of the shared contract: consent.js, the Portal's parser
   and scripts/check-consent-contract.mjs all require it. */
function initConsent(){
  ...
  if(bAccept) bAccept.onclick=()=>decide(true);
  if(bReject) bReject.onclick=()=>decide(false);
  if(bManage) bManage.onclick=()=>open(true);
  if(bSave) bSave.onclick=()=>decide(!!(adv&&adv.checked));
...
const bugitV16Faq = {
  en: [
    ['Does BugIt file automatically?', 'No. Every ticket, comment, attachment or notification that carries your report is previewed first. Before an irreversible filing, you approve it by typing FILE IT; chat text alone never files. A plain "yes" is not enough. Use dry run for zero-write practice. Connection tests are the one exception: when you deliberately run notify connect, notify test or notify doctor --live, BugIt sends one fixed test message without a preview, to the channel you name or, if you name none, to every channel you have switched on. Those tests carry no report content, and dry run blocks them too.'],
    ['Which trackers have built-in tested mapping?', 'All eleven include built-in, tested field mapping and create tickets directly through the tracker’s REST API, using a credential you create in your own account: Jira Cloud, Azure DevOps, GitHub Issues, GitLab Issues, Bugzilla, YouTrack, Linear, Shortcut, ClickUp, Asana and Trello. Filing does not use MCP. Crash tools and other services still connect through your own MCP server. Screenshot and log uploads through BugIt reach eight of the eleven; uploads to GitHub Issues, GitLab Issues and Bugzilla are not supported, and you are told before you try.'],
    ...
    ['What license/update data is sent?', 'BugIt sends Taskivator only licensing and update data: your Portal sign-in; an anonymous, one-way hashed device identifier; a random installation identifier; your device label and operating system; the BugIt version and selected plan; short-lived cryptographic challenge data; the one-time approval token from the Portal link your browser opened; a per-activation acknowledgement secret, whose hash is sent when you activate and whose value is sent only if you withdraw access for this device; and the entitlement approved for this device.'],
    ['What is included in Team?','BugIt Team is available now. A one-time payment covers a 1-year license for up to 5 members, and each member gets their own BugIt account, their own sign-in, and their own device activation, instead of a shared license. Team configuration is shared and managed centrally in the Portal, and it does not auto-renew. Solo is unaffected and available too. Need more than five? A second Team license adds five more seats to the same Team, and each license runs its own term.'],
  ...
  ja: [
    ['BugItは自動で登録しますか？','いいえ。レポートの内容を含むチケット、コメント、添付、通知はすべて、まずプレビューされます。取り消せない起票の前には、FILE IT と入力して承認します。チャットの文章だけでは登録されません。単なる「はい」では実行されません。ゼロ書き込みで練習するには dry run を使ってください。接続テストだけは例外です。お客様ご自身が notify connect、notify test、notify doctor --live を実行すると、BugIt は指定したチャンネルに、指定しない場合は有効なすべてのチャンネルに、固定のテストメッセージをプレビューなしで送信します。これらのテストにレポートの内容は含まれず、dry run でも送信されません。'],
    ['組み込みの検証済みマッピングがあるトラッカーは？','11 のトラッカーすべてが検証済みフィールドマッピングに対応し、お客様ご自身のアカウントで作成した資格情報を使って各トラッカーの REST API へ直接登録します（登録に MCP は使いません）: Jira Cloud、Azure DevOps、GitHub Issues、GitLab Issues、Bugzilla、YouTrack、Linear、Shortcut、ClickUp、Asana、Trello。クラッシュ解析ツールなどのサービスは、引き続きご自身の MCP サーバー経由で接続します。'],
    ...
    ['送信されるライセンス/更新データは？','ブラウザーの Portal で行う BugIt アカウントへのサインイン、一方向にハッシュ化された匿名のデバイス ID、ランダムなインストール ID、デバイス名（ホスト名）と OS 名、BugIt のバージョン、選択した Solo または Team のプランフィルター、短時間だけ有効な暗号学的チャレンジ情報、ブラウザーが開いた Portal のリンクに含まれる一回限りの承認トークン、アクティベーションごとの確認シークレット（アクティベーション時にそのハッシュ、シークレット自体はこのデバイスのアクセスを取り消すときのみ）、そしてこのデバイスに対して承認した Solo または Team のライセンス情報です。'],
    ['Teamには何が含まれますか？','BugIt Team は現在ご利用いただけます。買い切りで、最大5名向けの1年間ライセンスです。共有ライセンスではなく、各メンバーが自分専用の BugIt アカウント、自分専用のサインイン、自分専用のデバイスアクティベーションを持ちます。チーム設定は共有され、ポータルで一元管理され、自動更新はありません。Solo プランは影響を受けず、同様にご利用いただけます。 5名を超える場合は、2つ目の Team ライセンスを同じチームに追加すると席が5つ増えます。ライセンスごとに期間は別々です。'],
    ...
    ['ライセンスが切れるとどうなりますか？','ライセンス期間が終了すると3日間の猶予期間があり、その後は起票とアクティベーションに有効なライセンスが必要になります。ローカルのファイルと設定はそのまま残ります。ライセンスサーバー障害時は別で、キャッシュ確認により最大72時間オフラインで作業できます。1 年間は購入日に開始します。'],
  ...
  es: [
    ...
    ['¿Qué trackers tienen mapeo integrado y probado?','Los once tienen mapeo de campos integrado y probado y registran directamente a través de la API REST de cada tracker, con una credencial que creas en tu propia cuenta: Jira Cloud, Azure DevOps, GitHub Issues, GitLab Issues, Bugzilla, YouTrack, Linear, Shortcut, ClickUp, Asana y Trello. Registrar no usa MCP. Las herramientas de fallos y otros servicios siguen conectándose mediante tu propio servidor MCP.'],
    ...
    ['¿Qué datos de licencia/actualización se envían?','El inicio de sesión en tu cuenta de BugIt (en el navegador, en el Portal), un identificador de dispositivo con hash unidireccional, un identificador de instalación aleatorio, el nombre de tu equipo (hostname) y el del sistema operativo, la versión de BugIt y el filtro de plan Solo o Team que elijas, material criptográfico de desafío de corta duración, el token de aprobación de un solo uso del enlace del Portal que abrió tu navegador, un secreto de confirmación por activación (su hash al activar, y el valor en sí solo si revocas el acceso de este dispositivo) y la titularidad Solo o Team que apruebas para este dispositivo.'],
    ['¿Qué incluye Team?','El plan Team ya está disponible. Un pago único cubre una licencia de 1 año para hasta 5 miembros: cada miembro tiene su propia cuenta de BugIt, su propio inicio de sesión y su propia activación de dispositivo, en lugar de una licencia compartida. La configuración del equipo se comparte y se gestiona de forma centralizada en el Portal, y no se renueva automáticamente. El plan Solo no se ve afectado y también está disponible. ¿Necesitas más de cinco? Una segunda licencia de Team añade otros cinco puestos al mismo equipo, y cada licencia tiene su propio plazo.'],
    ...
    ['¿Qué pasa cuando expira mi licencia?','Tras finalizar la licencia anual hay un periodo de gracia de 3 días; después, las funciones de registro de tickets y activación requieren una licencia vigente, mientras que tus archivos y ajustes locales se conservan. Si falla el servidor de licencias, una verificación en caché permite seguir trabajando hasta 72 horas sin conexión. El año comienza en la fecha de compra.'],
  ...
  fr: [
    ...
    ['Quels trackers ont un mapping intégré et testé ?','Les onze ont un mapping de champs intégré et testé et créent les tickets directement via l’API REST de chaque outil, avec un identifiant que vous créez dans votre propre compte : Jira Cloud, Azure DevOps, GitHub Issues, GitLab Issues, Bugzilla, YouTrack, Linear, Shortcut, ClickUp, Asana et Trello. La création n’utilise pas MCP. Les outils de plantage et les autres services passent toujours par votre propre serveur MCP.'],
    ...
    ['Que se passe-t-il à l’expiration de ma licence ?','Une période de grâce de 3 jours suit la fin de la licence annuelle ; au-delà, les fonctions de création de tickets et d’activation nécessitent une licence en cours de validité, tandis que vos fichiers et paramètres locaux restent les vôtres. En cas de panne du serveur de licence, une vérification en cache vous laisse travailler jusqu’à 72 heures hors ligne. L’année débute à la date d’achat.'],
  ...
  de: [
    ['Legt BugIt automatisch Bugs an?','Nein. Jedes Ticket, jeder Kommentar, jeder Anhang und jede Benachrichtigung mit Ihrem Berichtsinhalt wird zuerst als Vorschau angezeigt. Vor einer irreversiblen Einreichung bestätigen Sie durch die Eingabe von FILE IT; Chat-Text allein reicht nie zum Einreichen. Ein einfaches „Ja“ reicht nicht aus. Nutzen Sie dry run, um ohne jeden Schreibvorgang zu üben. Verbindungstests sind die einzige Ausnahme: Wenn Sie notify connect, notify test oder notify doctor --live selbst ausführen, sendet BugIt ohne Vorschau eine feste Testnachricht an den Kanal, den Sie angeben, oder an jeden eingeschalteten Kanal, wenn Sie keinen angeben. Diese Tests enthalten keine Berichtsinhalte, und dry run blockiert sie ebenfalls.'],
    ['Welche Tracker haben integriertes, getestetes Mapping?','Alle elf haben integriertes, getestetes Feld-Mapping und reichen direkt über die eigene REST-API des Trackers ein, mit einer Zugangsberechtigung, die Sie in Ihrem eigenen Konto anlegen: Jira Cloud, Azure DevOps, GitHub Issues, GitLab Issues, Bugzilla, YouTrack, Linear, Shortcut, ClickUp, Asana und Trello. Das Einreichen nutzt kein MCP. Crash-Tools und weitere Dienste verbinden Sie weiterhin über Ihren eigenen MCP-Server.'],
    ...
    ['Welche Lizenz-/Update-Daten werden gesendet?','Die Anmeldung bei Ihrem BugIt-Konto (im Browser, im Portal), eine mit einem Einweg-Hash versehene Geräte-ID, eine zufällige Installationskennung, den Namen Ihres Geräts (Hostname) und des Betriebssystems, die BugIt-Version und den von Ihnen gewählten Solo- oder Team-Planfilter, kurzlebiges kryptografisches Challenge-Material, den einmaligen Freigabe-Token aus dem Portal-Link, den Ihr Browser geöffnet hat, ein Bestätigungsgeheimnis je Aktivierung (dessen Hash beim Aktivieren, der Wert selbst nur beim Widerruf des Zugriffs dieses Geräts) und die Solo- oder Team-Berechtigung, die Sie für dieses Gerät bestätigen.'],
    ['Was enthält Team?','Der Team-Tarif ist jetzt verfügbar. Eine Einmalzahlung deckt eine 1-Jahres-Lizenz für bis zu 5 Mitglieder ab, und jedes Mitglied erhält ein eigenes BugIt-Konto, eine eigene Anmeldung und eine eigene Geräteaktivierung statt einer gemeinsamen Lizenz. Die Team-Konfiguration wird gemeinsam genutzt und zentral im Portal verwaltet, ohne automatische Verlängerung. Der Solo-Tarif ist nicht betroffen und ebenfalls verfügbar. Mehr als fünf? Eine zweite Team-Lizenz fügt demselben Team fünf weitere Plätze hinzu, und jede Lizenz hat ihre eigene Laufzeit.'],
    ...
    ['Was passiert, wenn meine Lizenz abläuft?','Nach Ablauf der Jahreslizenz gilt eine Schonfrist von 3 Tagen; danach endet der Anspruch auf Einreichung und Aktivierung; Ihre lokalen Dateien und Einstellungen bleiben Ihnen erhalten. Bei Ausfall des Lizenzservers erlaubt die zwischengespeicherte Prüfung bis zu 72 Stunden Offline-Arbeit. Das Jahr beginnt am Kaufdatum.'],
  ...
  'pt-br': [
    ['O BugIt registra bugs automaticamente?','Não. Cada ticket, comentário, anexo ou notificação com o conteúdo do seu relato é visualizado primeiro. Antes de registrar um ticket irreversível, você aprova digitando FILE IT; o texto do chat sozinho nunca registra nada. Um simples "sim" não basta. Os testes de conexão são a única exceção: quando você mesmo executa notify connect, notify test ou notify doctor --live, o BugIt envia uma mensagem de teste fixa sem visualização prévia, para o canal que você indicar ou, se não indicar nenhum, para todos os canais que você tiver ativado. Esses testes não levam conteúdo do relato, e o dry run também os bloqueia.'],
    ['Quais trackers têm mapeamento integrado e testado?','Todos os onze têm mapeamento de campos integrado e testado e registram direto pela API REST de cada tracker, com uma credencial que você cria na sua própria conta: Jira Cloud, Azure DevOps, GitHub Issues, GitLab Issues, Bugzilla, YouTrack, Linear, Shortcut, ClickUp, Asana e Trello. Registrar não usa MCP. Ferramentas de travamento e outros serviços continuam se conectando pelo seu próprio servidor MCP.'],
    ...
    ['O que acontece quando minha licença expira?','Há um período de carência de 3 dias após o fim da licença anual; depois dele, as funções de registro de tickets e ativação exigem uma licença vigente, enquanto seus arquivos e configurações locais continuam sendo seus. Se o servidor de licenças cair, a verificação em cache permite até 72 horas de trabalho offline. O ano começa na data da compra.'],
  ...
  it: [
    ['BugIt crea bug automaticamente?','No. Ogni ticket, commento, allegato o notifica che contiene la tua segnalazione viene prima mostrato in anteprima. Prima di creare un ticket irreversibile, lo approvi digitando FILE IT; il solo testo della chat non crea mai nulla. Un semplice «sì» non basta. Usa dry run per esercitarti senza alcuna scrittura. I test di connessione sono la sola eccezione: quando esegui tu stesso notify connect, notify test o notify doctor --live, BugIt invia un messaggio di prova fisso senza anteprima, al canale che indichi o, se non ne indichi nessuno, a tutti i canali che hai attivato. Questi test non contengono contenuti della segnalazione e anche dry run li blocca.'],
    ['Quali tracker hanno mapping integrato e testato?','Tutti e undici hanno mapping dei campi integrato e testato e creano i ticket direttamente tramite l’API REST di ciascun tracker, con una credenziale che crei nel tuo account: Jira Cloud, Azure DevOps, GitHub Issues, GitLab Issues, Bugzilla, YouTrack, Linear, Shortcut, ClickUp, Asana e Trello. La creazione non usa MCP. Strumenti di crash e altri servizi si collegano ancora tramite il tuo server MCP.'],
    ...
    ['Quali dati di licenza/aggiornamento vengono inviati?','L’accesso al tuo account BugIt (nel browser, nel Portal), un ID dispositivo con hash a senso unico, un identificatore di installazione casuale, il nome del tuo computer (hostname) e quello del sistema operativo, la versione di BugIt e il filtro di piano Solo o Team che scegli, materiale crittografico di sfida di breve durata, il token di approvazione monouso contenuto nel link del Portal aperto dal tuo browser, un segreto di conferma per ogni attivazione (il suo hash all’attivazione, e il valore stesso solo in caso di revoca dell’accesso di questo dispositivo) e il diritto Solo o Team che approvi per questo dispositivo.'],
    ['Cosa include Team?','Il piano Team è disponibile ora. Un pagamento unico copre una licenza di 1 anno per un massimo di 5 membri: ogni membro ha il proprio account BugIt, il proprio login e la propria attivazione del dispositivo, invece di una licenza condivisa. La configurazione del team è condivisa e gestita centralmente nel Portale e non si rinnova automaticamente. Il piano Solo non è interessato ed è disponibile anch’esso. Servono più di cinque? Una seconda licenza Team aggiunge altre cinque postazioni allo stesso team, e ogni licenza ha il proprio periodo.'],
    ...
    ['Cosa succede quando la licenza scade?','Dopo il termine annuale c’è un periodo di grazia di 3 giorni, al termine del quale le funzioni di creazione dei ticket e di attivazione richiedono una licenza valida; i file e le impostazioni locali restano tuoi. Se il server licenze non è raggiungibile, la verifica in cache permette fino a 72 ore di lavoro offline. L’anno decorre dalla data di acquisto.'],
  ...
  ko: [
    ['BugIt이 자동으로 버그를 등록하나요?','아니요. 보고 내용이 담긴 모든 티켓, 댓글, 첨부, 알림은 먼저 미리보기로 표시됩니다. 되돌릴 수 없는 티켓 등록 전에는 FILE IT을 입력해 승인해야 하며, 채팅 문구만으로는 등록되지 않습니다. 단순히 "예"만으로는 진행되지 않습니다. 연결 테스트만 예외입니다. 사용자가 직접 notify connect, notify test, notify doctor --live 를 실행하면 BugIt이 지정한 채널로, 지정하지 않으면 켜 둔 모든 채널로 고정된 테스트 메시지를 미리보기 없이 보냅니다. 이 테스트에는 보고 내용이 담기지 않으며 dry run에서도 차단됩니다.'],
    ['내장 검증 매핑이 있는 트래커는 무엇인가요?','열한 개 모두 내장 검증 필드 매핑을 제공하며, 직접 만든 자격 증명으로 각 추적기의 REST API에 바로 등록합니다: Jira Cloud, Azure DevOps, GitHub Issues, GitLab Issues, Bugzilla, YouTrack, Linear, Shortcut, ClickUp, Asana, Trello. 등록에는 MCP를 사용하지 않습니다. 크래시 도구 등 다른 서비스는 여전히 사용자의 MCP 서버를 통해 연결합니다.'],
    ...
    ['Taskivator가 제 작업을 보나요?','아니요. 버그 보고서, 사양, 용어집, 스크린샷, 코드, 설정, 티켓은 Taskivator로 전송되지 않습니다. 라이선스/업데이트 데이터만 사용됩니다.'],
    ['어떤 라이선스/업데이트 데이터가 전송되나요?','브라우저의 Portal에서 이루어지는 BugIt 계정 로그인, 단방향 해시된 기기 ID, 무작위 설치 식별자, 기기 이름(호스트명)과 운영체제 이름, BugIt 버전과 선택한 Solo 또는 Team 플랜 필터, 수명이 짧은 암호화 챌린지 자료, 브라우저가 연 Portal 링크에 들어 있는 일회용 승인 토큰, 활성화마다 생성되는 확인 비밀값(활성화 시 그 해시, 값 자체는 이 기기의 접근을 해제할 때만), 그리고 이 기기에 대해 승인하는 Solo 또는 Team 권한입니다.'],
    ['Team에는 무엇이 포함되나요?','BugIt Team을 지금 이용할 수 있습니다. 1회 결제로 최대 5명을 위한 1년 라이선스를 제공합니다. 공유 라이선스 대신 각 구성원이 자신의 BugIt 계정, 자신의 로그인, 자신의 기기 활성화를 갖습니다. 팀 구성은 공유되며 포털에서 중앙 관리되고, 자동 갱신이 없습니다. Solo 요금제는 영향을 받지 않으며 함께 이용할 수 있습니다. 다섯 명이 넘나요? 같은 팀에 두 번째 Team 라이선스를 추가하면 좌석이 5개 늘어나며, 라이선스마다 기간은 각각입니다.'],
    ['등록 없이 연습할 수 있나요?','네. dry run을 사용하면 티켓, 댓글, 첨부, 알림을 만들지 않고 전체 보고서를 생성할 수 있습니다. dry run은 BugIt에 포함된 도우미의 쓰기를 막고 에이전트에 트래커 쓰기를 거부하도록 지시합니다. 이 거부는 플랫폼 잠금이 아니라 BugIt의 지시를 따르므로, 평가할 때는 읽기 전용 자격 증명을 사용하세요.'],
    ...
    ['라이선스가 만료되면 어떻게 되나요?','라이선스 기간이 끝나면 3일의 유예 기간이 적용되며, 그 이후 등록과 활성화에는 유효한 라이선스가 필요합니다. 로컬 파일과 설정은 그대로 유지됩니다. 라이선스 서버 장애 시에는 캐시된 검증으로 최대 72시간 오프라인 작업이 가능합니다. 1년은 구매일부터 시작됩니다.'],
  ...
  zh: [
    ['BugIt 会自动提交缺陷吗？','不会。每个包含报告内容的工单、评论、附件或通知都会先预览。执行不可撤销的工单提交前，需要输入 FILE IT 进行批准；仅靠聊天文字永远不会提交。仅输入"是"不会执行操作。连接测试是唯一的例外：当你自己运行 notify connect、notify test 或 notify doctor --live 时，BugIt 会不经预览地发送一条固定的测试消息，发送到你指定的渠道；若未指定，则发送到你已启用的所有渠道。这些测试不含报告内容，dry run 同样会阻止它们。'],
    ['哪些跟踪器有内置且已测试的字段映射？','全部十一个都提供内置且已测试的字段映射，并使用你在自己账户中创建的凭据，直接通过各自的 REST API 提交：Jira Cloud、Azure DevOps、GitHub Issues、GitLab Issues、Bugzilla、YouTrack、Linear、Shortcut、ClickUp、Asana 和 Trello。提交不使用 MCP。崩溃工具等其他服务仍通过你自己的 MCP 服务器连接。'],
    ...
    ['会发送哪些许可证/更新数据？','在浏览器 Portal 中登录你的 BugIt 账户、一个单向哈希的设备 ID、一个随机安装标识符、你的设备名（主机名）与操作系统名称、BugIt 版本与你选择的 Solo 或 Team 套餐筛选、短时有效的加密质询材料、浏览器打开的 Portal 链接中的一次性批准令牌、每次激活生成的确认密钥（激活时发送其哈希；仅在撤销这台设备的访问时才发送密钥本身），以及你为这台设备批准的 Solo 或 Team 权益。'],
    ['Team 包含什么？','BugIt Team 现已推出。一次性付款即可获得适用于最多 5 名成员的 1 年期许可证：每位成员都拥有各自的 BugIt 账户、各自的登录和各自的设备激活，而不是共享一个许可证。团队配置为共享，并在门户中集中管理，且不自动续订。Solo 套餐不受影响，同样可用。 超过五人？给同一个团队再加一个 Team 许可证即可增加 5 个席位，每个许可证各自计算期限。'],
    ...
    ['许可证到期后会怎样？','许可证期限结束后有 3 天宽限期，此后提交和激活功能需要有效的许可证。你的本地文件和设置仍归你所有。许可证服务器故障是另一种情况：缓存验证允许最多 72 小时离线继续工作。该年度自购买之日起计算。'],
  ...
  ru: [
    ['BugIt автоматически создает баги?','Нет. Каждый тикет, комментарий, вложение и уведомление с содержимым вашего отчёта сначала показывается для предпросмотра. Перед необратимым созданием тикета вы подтверждаете его вводом FILE IT; один только текст в чате никогда ничего не создаёт. Обычного «да» недостаточно. Используйте dry run, чтобы тренироваться без записи. Проверки подключения составляют единственное исключение: когда вы сами запускаете notify connect, notify test или notify doctor --live, BugIt без предпросмотра отправляет одно фиксированное тестовое сообщение в указанный вами канал или, если вы не укажете ни одного, во все включённые каналы. Эти проверки не содержат данных отчёта, и dry run тоже их блокирует.'],
    ...
    ['Какие данные лицензии/обновлений отправляются?','Вход в вашу учётную запись BugIt (в браузере, на Portal), односторонний хешированный идентификатор устройства, случайный идентификатор установки, имя вашего устройства (hostname) и название операционной системы, версию BugIt и выбранный вами фильтр плана Solo или Team, недолговечный криптографический материал запроса-ответа, одноразовый токен подтверждения из ссылки Portal, которую открыл ваш браузер, секрет подтверждения для каждой активации (его хеш при активации, а сам секрет только при отзыве доступа этого устройства), а также право Solo или Team, которое вы подтверждаете для этого устройства.'],
    ['Что входит в Team?','Тариф Team уже доступен. Разовый платёж включает лицензию на 1 год для команды до 5 участников, у каждого участника собственный аккаунт BugIt, собственный вход и собственная активация устройства вместо общей лицензии. Конфигурация команды общая и управляется централизованно в Портале, без автопродления. Тариф Solo не затронут и также доступен. Нужно больше пяти? Вторая лицензия Team добавляет той же команде ещё пять мест, и у каждой лицензии свой срок.'],
    ...
    ['Как работают обновления?','BugIt создает резервную копию ваших настроек, проверяет подписанные обновления, устанавливает их на месте и сохраняет конфигурацию, глоссарий, стиль оформления, лицензию и подключения.'],
    ['Что происходит после истечения лицензии?','После окончания годового срока есть 3-дневный льготный период. Если сервер лицензий недоступен, кэшированная проверка позволяет работать офлайн до 72 часов. Год начинается с даты покупки.'],
...
/* =========================================================================
   ASK BUGIT  -  the hero bar that opens the Guide (2026-09-20)

   Replaces the old "BugIt QA Agent" pill. Two jobs: look like somewhere you
   can type, and actually open the Guide when pressed.

   The typing is decoration. The accessible name is fixed (hero.askCta) and
   the rotating text is aria-hidden, so nothing a screen reader or a voice
   control user relies on ever changes under them.
   ========================================================================= */
(function(){
  var bar=document.getElementById('askBar');
  if(!bar) return;
  var out=document.getElementById('askBarQ');
  var caret=bar.querySelector('.askbar-caret');
  if(!out) return;

  var reduce=false;
  try{ reduce=window.matchMedia('(prefers-reduced-motion: reduce)').matches }catch(e){}

  /* Below 384px styles.css drops the question entirely: it cannot be made to fit in eleven
     languages at that width. Typing into a display:none box would still run a timer every
     40ms on the weakest phones we serve, so the rotation stands down there instead. Queried,
     not measured once: a phone that rotates crosses this line. */
  var narrowMq=null;
  try{ narrowMq=window.matchMedia('(max-width:383px)') }catch(e){}
  function hiddenHere(){ return !!(narrowMq&&narrowMq.matches) }

  var qs=[],qi=0,ci=0,phase='type',timer=null,onScreen=true;

  function questions(){
    var d=(typeof i18n!=='undefined'&&i18n[currentLang])?i18n[currentLang]:(typeof i18n!=='undefined'?i18n.en:null);
    var h=(d&&d.hero)||{};
    var list=[h.askQ1,h.askQ2,h.askQ3].filter(function(x){return typeof x==='string'&&x});
    if(!list.length&&typeof i18n!=='undefined'&&i18n.en&&i18n.en.hero){
      var e=i18n.en.hero; list=[e.askQ1,e.askQ2,e.askQ3].filter(Boolean);
    }
    return list;
  }
  ...
  /* Reduced motion, or a tab nobody is looking at: show a whole question and
     stop. The bar still reads correctly; it just does not perform. */
  function still(){
    stop();
    out.textContent=qs.length?qs[qi%qs.length]:'';
    if(caret)caret.style.display=reduce?'none':'';
  }
  ...
  function step(){
    ...
    if(phase==='type'){
      ...
      if(ci>=q.length){ phase='hold'; timer=setTimeout(step,2100); return }
      timer=setTimeout(step,38+Math.random()*34);
      return;
    }
    if(phase==='hold'){ phase='erase'; timer=setTimeout(step,20); return }
    ci-=2;
    if(ci<=0){ ci=0; qi=(qi+1)%qs.length; phase='type'; out.textContent=''; timer=setTimeout(step,260); return }
    out.textContent=q.slice(0,ci);
  ...
  function run(){
    stop();
    if(!qs.length){ out.textContent=''; return }
    if(hiddenHere()){ stop(); return }
    if(reduce||document.visibilityState==='hidden'||!onScreen){ still(); return }
    if(caret)caret.style.display='';
    phase='type'; ci=0; out.textContent='';
    timer=setTimeout(step,420);
  }

  /* Called by applyLang. The questions are per language, so a language change
     restarts the rotation rather than finishing the old sentence in the new
     language, which is what a naive re-read would do. */
  window.__askRelocalize=function(){
    var next=questions();
    var changed=next.join('\u0001')!==qs.join('\u0001');
    qs=next;
    if(changed){ qi=0; run() }
  };

  document.addEventListener('visibilitychange',function(){
    if(document.visibilityState==='hidden'){ stop() } else { run() }
  });

  /* Rotate a phone across 384px and the question appears or disappears; the rotation has to
     follow it, or it comes back to a visible box that never types again. */
  if(narrowMq){
    var onNarrow=function(){ run() };
    ...
    try{
      new IntersectionObserver(function(es){
        for(var i=0;i<es.length;i++){
          var vis=es[i].isIntersecting;
          if(vis===onScreen) continue;
          onScreen=vis;
          if(vis) run(); else stop();
        }
      },{threshold:0.01}).observe(bar);
    }catch(e){}
  }

  /* ---- opening the Guide ------------------------------------------------
     The Guide refuses to open over the consent banner, and on a first visit
     that banner is up, which is exactly when this bar is most likely to be
     pressed. A control that silently does nothing is worse than no control,
     so in that case send the visitor to the choice that is blocking them. */
  function focusConsent(){
    var banner=document.getElementById('consentBanner');
    if(!banner||!banner.getClientRects().length) return false;
    var target=document.getElementById('consentAccept')||banner.querySelector('button');
  ...
  var wantOpen=false;
  function openGuide(){
    var g=window.BugitGuide;
    if(g&&typeof g.open==='function'){
      if(g.open()) return true;
      focusConsent();          /* refused: the banner is the reason */
      return false;
    }
    /* The Guide module has not finished loading. Ask for it once it says it
       is ready rather than clicking a launcher that may not exist yet. */
    var launcher=document.querySelector('.bgd-launch');
    if(launcher){ launcher.click(); return true }
    wantOpen=true;
    return false;
  }

  document.addEventListener('bugit-guide:ready',function(){
    if(!wantOpen) return;
    wantOpen=false;
    if(window.BugitGuide&&!window.BugitGuide.open()) focusConsent();
  });

  bar.addEventListener('click',function(e){ e.preventDefault(); openGuide() });
