...
const reqFaq={
 ...
 ko:["BugIt를 사용하려면 무엇이 필요한가요?","BugIt는 Visual Studio Code에서 GitHub Copilot Chat, Claude 확장, 또는 다른 어시스턴트와 최신 버전의 Python(3.10-3.14)과 함께 실행됩니다. Windows 11은 완전히 지원되며 출시 검증을 마쳤습니다. macOS와 Linux도 지원되지만, 검증 범위를 넓히는 동안 현재는 프리뷰 상태입니다. BugIt에는 AI 모델이나 구독이 포함되어 있지 않으며, 직접 준비한 것을 사용합니다."],
...
const bugitV16Faq = {
  en: [
    ...
    ['Which trackers have built-in tested mapping?', 'All eleven include built-in, tested field mapping and create tickets directly through the tracker’s REST API, using a credential you create in your own account: Jira Cloud, Azure DevOps, GitHub Issues, GitLab Issues, Bugzilla, YouTrack, Linear, Shortcut, ClickUp, Asana and Trello. Filing does not use MCP. Crash tools, test management and other services still connect through your own MCP server. Screenshot and log uploads reach eight of the eleven: GitHub Issues, GitLab Issues and Bugzilla accept no file from any tool, and BugIt tells you before you try.'],
  ...
  es: [
    ...
    ['¿Qué pasa cuando expira mi licencia?','Hay un periodo de gracia de 3 días tras terminar la licencia anual. Si falla el servidor de licencias, una verificación en caché permite seguir trabajando hasta 72 horas sin conexión. El año comienza en la fecha de compra.'],
  ...
  fr: [
    ...
    ['Que se passe-t-il à l’expiration de ma licence ?','Une période de grâce de 3 jours suit la fin de la licence annuelle. En cas de panne du serveur de licence, une vérification en cache vous laisse travailler jusqu’à 72 heures hors ligne. L’année débute à la date d’achat.'],
  ...
  'pt-br': [
    ...
    ['O que acontece quando minha licença expira?','Há um período de carência de 3 dias após o fim da licença anual. Se o servidor de licenças cair, a verificação em cache permite até 72 horas de trabalho offline. O ano começa na data da compra.'],
  ...
  it: [
    ...
    ['Cosa succede quando la licenza scade?','Dopo il termine annuale c’è un periodo di grazia di 3 giorni. Se il server licenze non è raggiungibile, la verifica in cache permette fino a 72 ore di lavoro offline. L’anno decorre dalla data di acquisto.'],
