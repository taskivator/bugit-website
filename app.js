...
const bugitV16Faq = {
  en: [
    ['Does BugIt file automatically?', 'No. Every ticket, comment, attachment or notification that carries your report is previewed first. Before an irreversible filing, you approve it by typing FILE IT; chat text alone never files. A plain "yes" is not enough. Use dry run for zero-write practice. Connection tests are the one exception: when you deliberately run notify connect, notify test or notify doctor --live, BugIt sends one fixed test message to the channel you are setting up, without a preview. Those tests carry no report content, and dry run blocks them too.'],
  ...
  ja: [
    ['BugItは自動で登録しますか？','いいえ。レポートの内容を含むチケット、コメント、添付、通知はすべて、まずプレビューされます。取り消せない起票の前には、FILE IT と入力して承認します。チャットの文章だけでは登録されません。単なる「はい」では実行されません。ゼロ書き込みで練習するには dry run を使ってください。接続テストだけは例外です。お客様ご自身が notify connect、notify test、notify doctor --live を実行すると、BugIt は設定中のチャンネルに固定のテストメッセージをプレビューなしで送信します。これらのテストにレポートの内容は含まれず、dry run でも送信されません。'],
  ...
  ko: [
    ['BugIt이 자동으로 버그를 등록하나요?','아니요. 보고 내용이 담긴 모든 티켓, 댓글, 첨부, 알림은 먼저 미리보기로 표시됩니다. 되돌릴 수 없는 티켓 등록 전에는 FILE IT을 입력해 승인해야 하며, 채팅 문구만으로는 등록되지 않습니다. 단순히 "예"만으로는 진행되지 않습니다. 연결 테스트만 예외입니다. 사용자가 직접 notify connect, notify test, notify doctor --live 를 실행하면 BugIt이 설정 중인 채널로 고정된 테스트 메시지를 미리보기 없이 보냅니다. 이 테스트에는 보고 내용이 담기지 않으며 dry run에서도 차단됩니다.'],
  ...
  zh: [
    ['BugIt 会自动提交缺陷吗？','不会。每个包含报告内容的工单、评论、附件或通知都会先预览。执行不可撤销的工单提交前，需要输入 FILE IT 进行批准；仅靠聊天文字永远不会提交。仅输入"是"不会执行操作。连接测试是唯一的例外：当您自己运行 notify connect、notify test 或 notify doctor --live 时，BugIt 会向您正在配置的渠道发送一条固定的测试消息，不经预览。这些测试不含报告内容，dry run 同样会阻止它们。'],
  ...
  ru: [
    ['BugIt автоматически создает баги?','Нет. Каждый тикет, комментарий, вложение и уведомление с содержимым вашего отчёта сначала показывается для предпросмотра. Перед необратимым созданием тикета вы подтверждаете его вводом FILE IT; один только текст в чате никогда ничего не создаёт. Обычного «да» недостаточно. Используйте dry run, чтобы тренироваться без записи. Проверки подключения составляют единственное исключение: когда вы сами запускаете notify connect, notify test или notify doctor --live, BugIt отправляет одно фиксированное тестовое сообщение в настраиваемый канал без предпросмотра. Эти проверки не содержат данных отчёта, и dry run тоже их блокирует.'],
