...
add('zh',merge(i18n.ja,{name:'中文',nav:{features:'功能',integrations:'集成',pricing:'价格',docs:'文档',faq:'FAQ'},cta:{early:'获取 BugIt',demo:'观看演示'},hero:{title:'会学习你工作流的 <span>QA 代理</span>。',subtitle:'把粗略测试笔记变成清晰、已审阅的缺陷单，并在几秒内提交到你的跟踪器。'},pricing:{soloTitle:'单人许可',teamTitle:'团队许可',perYear:' 一次性付款',soloTerm:'1 年期 Solo 授权 · 不自动续订',teamTerm:'1 年期 Team 许可证 · 不自动续订',limited:'尝鲜价',soloRegular:'常规价格 $59.99',teamRegular:'常规价格 $249.99',teamCta:'获取 BugIt Team',soloCta:'获取 BugIt Solo'},faq:{title:'购买前，先把问题说清楚。',items:[['BugIt 会自动提交缺陷吗？','不会。每个包含报告内容的工单、评论、附件或通知都会先预览。执行不可撤销的工单提交前，需要输入 FILE IT 进行批准；仅靠聊天文字永远不会提交。仅输入"是"不会执行操作。连接测试是唯一的例外：当您自己运行 notify connect、notify test 或 notify doctor --live 时，BugIt 会不经预览地发送一条固定的测试消息，发送到您指定的渠道；若未指定，则发送到您已启用的所有渠道。这些测试不含报告内容，dry run 同样会阻止它们。'],['哪些跟踪器有内置测试映射？','全部十一个跟踪系统都内置经过测试的字段映射，BugIt 使用你在自己账户中创建的凭据向其中每一个提交：Jira Cloud、Azure DevOps、GitHub Issues、GitLab Issues、Bugzilla、YouTrack、Linear、Shortcut、ClickUp、Asana 和 Trello。在保存连接之前，BugIt 会先验证你的凭据和目标位置。崩溃分析工具、测试管理等其他服务通过你自己的 MCP 服务器连接，供助手读取。'],['我需要 GitHub Copilot 吗？','推荐使用，也是最简单的方式。BugIt 也可以使用你自己的 OpenAI 或 Anthropic 密钥独立运行。'],['可以使用 Claude、Gemini 或 GPT 吗？','可以。在 GitHub Copilot 中使用你的计划可用的模型。独立模式支持 OpenAI 或 Anthropic。'],['Taskivator 会看到我的工作吗？','不会。缺陷报告、规格、术语表、截图和设置不会发送给 Taskivator。'],['Team 包含什么？','BugIt Team 现已推出。一次性付款即可获得适用于最多 5 名成员的 1 年期许可证：每位成员都拥有各自的 BugIt 账户、各自的登录和各自的设备激活，而不是共享一个许可证。团队配置为共享，并在门户中集中管理，且不自动续订。Solo 套餐不受影响，同样可用。 超过五人？给同一个团队再加一个 Team 许可证即可增加 5 个席位，每个许可证各自计算期限。'],['可以不提交就练习吗？','可以。使用 dry run 可生成完整报告，而不会创建工单、评论或通知。 dry run 会阻止 BugIt 内置的辅助工具写入，并指示代理拒绝对跟踪器的写入；这种拒绝遵循的是 BugIt 的指令而非平台级锁定，因此评估时请使用只读凭据。'],['可以自定义 BugIt 吗？','可以。你可以配置术语表、分类、平台、团队风格和跟踪器设置。'],['BugIt 支持截图吗？','支持。BugIt 会在你的电脑上打开一个文件夹，用于放置截图、录屏和日志，并按你的缺陷跟踪工具进行准备，在你批准后上传。粘贴到聊天中的图片可以被读取，但无法作为附件上传，因此 BugIt 使用文件夹。11 个跟踪工具中有 8 个支持上传。'],['更新如何工作？','BugIt 会备份设置、验证签名更新，并保留你的配置。'],['许可证到期会怎样？','你的本地文件和设置会保留。提交功能需要有效许可证。'],['如何获得支持？','先让 AI 助手诊断问题，然后运行 Check status 或 Check readiness。如果仍需帮助，请从 BugIt 仪表盘提交支持工单，无需包含机密项目信息。']]},docs:{getting:'快速开始',user:'用户指南',license:'许可协议',privacy:'隐私政策',support:'支持',supportDesc:'获取帮助并提交支持工单。'}}));
...
const bugitV16Faq = {
  ...
  zh: [
    ['BugIt 会自动提交缺陷吗？','不会。每个包含报告内容的工单、评论、附件或通知都会先预览。执行不可撤销的工单提交前，需要输入 FILE IT 进行批准；仅靠聊天文字永远不会提交。仅输入"是"不会执行操作。连接测试是唯一的例外：当您自己运行 notify connect、notify test 或 notify doctor --live 时，BugIt 会不经预览地发送一条固定的测试消息，发送到您指定的渠道；若未指定，则发送到您已启用的所有渠道。这些测试不含报告内容，dry run 同样会阻止它们。'],
