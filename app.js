...
const bugitV16Faq = {
  en: [
    ...
    ['What license/update data is sent?', 'BugIt sends Taskivator only licensing and update data: your Portal sign-in; an anonymous, one-way hashed device identifier; a random installation identifier; your device label and operating system; the BugIt version and selected plan; short-lived cryptographic challenge data; a per-activation acknowledgement secret, whose hash is sent when you activate and whose value is sent only if you withdraw access for this device; and the entitlement approved for this device.'],
  ...
  ja: [
    ...
    ['送信されるライセンス/更新データは？','ブラウザーの Portal で行う BugIt アカウントへのサインイン、一方向にハッシュ化された匿名のデバイス ID、ランダムなインストール ID、デバイス名（ホスト名）と OS 名、BugIt のバージョン、選択した Solo または Team のプラン、短時間だけ有効な暗号学的チャレンジ情報、アクティベーションごとの確認シークレット（アクティベーション時にそのハッシュ、シークレット自体はこのデバイスのアクセスを取り消すときのみ）、そしてこのデバイスに対して承認した Solo または Team のライセンス情報です。'],
  ...
  de: [
    ...
    ['Welche Lizenz-/Update-Daten werden gesendet?','Die Anmeldung bei Ihrem BugIt-Konto (im Browser, im Portal), eine mit einem Einweg-Hash versehene Geräte-ID, eine zufällige Installationskennung, den Namen Ihres Geräts (Hostname) und des Betriebssystems, die BugIt-Version und den von Ihnen gewählten Solo- oder Team-Planfilter, kurzlebiges kryptografisches Challenge-Material, ein Bestätigungsgeheimnis je Aktivierung (dessen Hash beim Aktivieren, der Wert selbst nur beim Widerruf des Zugriffs dieses Geräts) und die Solo- oder Team-Berechtigung, die Sie für dieses Gerät bestätigen.'],
  ...
  ko: [
    ...
    ['어떤 라이선스/업데이트 데이터가 전송되나요?','브라우저의 Portal에서 이루어지는 BugIt 계정 로그인, 단방향 해시된 기기 ID, 무작위 설치 식별자, 기기 이름(호스트명)과 운영체제 이름, BugIt 버전과 선택한 Solo 또는 Team 플랜 필터, 수명이 짧은 암호화 챌린지 자료, 활성화마다 생성되는 확인 비밀값(활성화 시 그 해시, 값 자체는 이 기기의 접근을 해제할 때만), 그리고 이 기기에 대해 승인하는 Solo 또는 Team 권한입니다.'],
  ...
  zh: [
    ...
    ['会发送哪些许可证/更新数据？','在浏览器 Portal 中登录你的 BugIt 账户、一个单向哈希的设备 ID、一个随机安装标识符、你的设备名（主机名）与操作系统名称、BugIt 版本与你选择的 Solo 或 Team 套餐筛选、短时有效的加密质询材料、每次激活生成的确认密钥（激活时发送其哈希；仅在撤销这台设备的访问时才发送密钥本身），以及你为这台设备批准的 Solo 或 Team 权益。'],
